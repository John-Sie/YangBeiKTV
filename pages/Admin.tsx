...
export const Admin: React.FC<AdminProps> = ({ songs, requests, users, refreshData }) => {
    ...
    const handleSaveUserForm = async (e: React.FormEvent) => {
        ...
        try {
            ...
            refreshData(); // Added refreshData to update the user list
        } catch (err: any) {
    ...
    const handleConfirmDeleteUser = async () => {
        ...
        try {
            ...
            // We can't strictly rely on `success` returned from deleteUser since it might throw
            showToast('用戶及其相關資料已完整刪除', 'success');
            refreshData(); // Added refreshData to update the list after deletion
        } catch (error) {
                    ...
                    <div className="space-y-4">
                        <div className="flex flex-col md:flex-row gap-2 justify-between">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                ...
                                <label className={`flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-bold cursor-pointer transition-colors ${isProcessing ? 'bg-purple-400 cursor-not-allowed text-white' : 'bg-purple-600 hover:bg-purple-700 text-white'
                                    }`}>
