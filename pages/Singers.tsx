...
const KNOWN_CHORUS = new Set([
    ...
    '七朵花 183CLUB', '八三夭 A-Lin', '小宇 A-Lin', '呂方 A-Lin'
]);
...
const KNOWN_MALE = new Set([
    ...
    '巴大雄', '祁隆', '吳聽徹', '巫奇', '林奕匡', '派偉俊', '焦靖峰', 
    '陳雅森', '乱彈阿翔', '倪賓', '陳旭', '李明德',
    '丁子峻', '卜學亮', '小沈陽', '孔令奇', '王宏恩', '成龍', '朱孝天', '朴樹', 
    '江宏恩', '中孝介', '扎西頓珠', '民雄', '民雄巴萊', '包小柏', '小蟲', '山野',
...
const KNOWN_FEMALE = new Set([
    ...
    '多哇.才吉', '劉藍溪', '凡妮絲', '八代亞紀', '三笠優子', '三澤あけみ', '大月みやこ', '大西結花', 
    '大和田りつこ', '小林幸子', '小林明子', '小柳ルミ子', '山崎ハコ', '内田有紀', '加藤登紀子', 
    '北野まち子', '市川由紀乃', '本多美', '山口ひろみ',
    '丁芙妮', '九姨太', '于冬然', '于楓', '于櫻櫻', '大泫', '大芭', '大籽', '小球', '小霞', 
    '山崎洋子', '不是花火呀', '王一一', '王一佳', '王七七', '冉小冉', '包美聖',
...
const KNOWN_BANDS = new Set([
    ...
    '耳朵便利店', '東方T&J', '10cc', '五堅情', 'W0LF(S)', 'Girls’ Generation', 
    '内山田洋とクール・ファイブ', '伍佰 China Blue', '山水組合', '化學猴子', '中堅份子',
    '丁香曉曉', '三十萬年老虎鉗', '三角COOL', '三個人', '丸子', '木吉他', '山風點伙', 
    '大小姐', '火星熊', '北原山貓', '兄弟匯',
...
const KNOWN_OTHERS = new Set([
    ...
    '今夜又灑下雨', '天涯我獨行', '太魯閣歌曲', '日晚會用', '月夜愁', '月娘晚安', '水長流',
    '世界多美麗', '另一種鄉愁', '可愛的老馬', '可憐彼個小姑娘', '可憐的小姑娘', '可憐的純情花',
    '可憐酒家女', '台灣女孩', '平安夜', '正午陽光', '再會啦初戀', '吃頭路人', '行船的人',
    '我是行船人', '何日君再來', '初戀', '忘年計畫', '快樂的馬車', '我有一句話', '河邊春夢',
    '命運青紅燈', '夜雨恩情', '孤女的願望', '幸福在這裡', '思想起', '昨夜星辰', '珍藏版',
    '祈禱', '祝你順風', '情人的黃襯衫', '情網', '望郎早歸', '熱情的曼波', '黃昏的故鄉',
    '黃昏的海邊', '愛你一萬年', '愛你在心口難開', '愛你入骨', '愛的禮物', '愛的憧憬', '愛的小路',
    '愛不是佔有', '愛某不驚甘苦', '愛突擊', '愛苗', '落大雨彼一日', '葡萄不憤怒',
    '路燈了解我心意', '過去的春夢', '懷念的播音員', '難忘的人', '霧夜的燈塔', '藏族民歌',
    '憑欄聽雨', '償還', '偷吻', '問題回答',
    '179CLUB', '181CLUB', '182CLUB', '184', '4J', '6y17',
    '一文字辰也', '一玟', '丁力', '丁于', '丁衣凡', '丁肆 Dicey',
    '七字輩', '二胖u', '卜心',
    '三山ひろし', '三門忠司', '千昌夫',
    '上豪', '也可', '于立成', '于浩威', '于晨',
    '内山和尚', '内山姑娘', '内山姑娘要出嫁',
    '大壯', '大哲', '大豪', '大歡', '大湖MUTA Tipsy',
    '子非魚', '子堯',
    '小久', '小山', '小文', '小曼', '小莊', '小魚', '小魚兒', '小曾', '小樂哥', '小龍女', '小藍背心', '小魔女', '小鶯歌',
    '山川豊', '山内惠介', '山本譲二', '山本讓二', '山姝', '山K',
    '火星', '火星人',
    '北京山貓', '北京妞貓',
    '心月', '心宜', '心瑤', '心影',
    '付玉龍', '代小波', '代野',
